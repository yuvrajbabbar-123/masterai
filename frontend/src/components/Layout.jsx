...
import {
  ...
  Trophy, UserCircle, CreditCard, SignOut, Flame, Lightning, FileText, List,
...
function NavContents({ onNavigate, doLogout }) {
  return (
    <>
      ...
      <div className="p-3 border-t border-white/10 space-y-1">
        <NavLink to="/app/subscription" onClick={onNavigate} data-testid="nav-upgrade"
...
export default function Layout() {
  ...
  const isTyping = location.pathname.includes("/typing") || location.pathname.includes("/learn/") || location.pathname.includes("/documents/");
  ...
  return (
    <div className="min-h-screen flex bg-[#0A0A0A] relative z-10">
      ...
      <aside className={`hidden md:flex flex-col w-64 shrink-0 border-r border-white/10 bg-[#0D0D0D] fixed h-screen transition-opacity duration-500 ${isTyping ? "opacity-40 hover:opacity-100" : "opacity-100"}`}>
        <NavContents onNavigate={() => {}} doLogout={doLogout} />
      </aside>

      {/* Mobile drawer */}
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        <SheetContent side="left" className="p-0 w-72 bg-[#0D0D0D] border-white/10 flex flex-col">
          <NavContents onNavigate={() => setMobileOpen(false)} doLogout={doLogout} />
        </SheetContent>
      </Sheet>

      {/* Main */}
      <div className="flex-1 md:ml-64 flex flex-col min-h-screen">
        <header className={`sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-[#0A0A0A]/80 backdrop-blur-md border-b border-white/10 transition-opacity duration-500 ${isTyping ? "opacity-60 hover:opacity-100" : "opacity-100"}`}>
          <div className="flex items-center gap-3">
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              <SheetTrigger asChild>
                <button className="md:hidden p-1.5 -ml-1 text-[#A3A3A3] hover:text-white" data-testid="mobile-menu-button" aria-label="Open menu">
        ...
        <main className="flex-1 p-5 sm:p-10 max-w-7xl w-full mx-auto">
          <Outlet />
        </main>
      </div>
    </div>
